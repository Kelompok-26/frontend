<template>
  <div class="flex flex-col gap-20 w-full h-screen mx-auto py-10">
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
      <table
        class="w-full text-left text-black min-h-fit border-[1px] border-black/50"
      >
        <thead class="uppercase">
          <tr>
            ...
            <th scope="col" class="px-6 py-6">Provider</th>
            <th scope="col" class="px-6 py-6">{{ tipe }}</th>
            <th scope="col" class="px-6 py-6">Harga</th>
        ...
        <tbody>
          ...
          >
            ...
            <td class="px-6 py-4">{{ val.provider_name }}</td>
            <td class="px-6 py-4">{{ val.product_name }}</td>
            <td class="px-6 py-4">{{ val.nominal }}</td>
            <td class="px-6 py-4">{{ val.stock }} stok</td>
...
export default {
  ...
  methods: {
    ...
    deleteUser(id) {
      const config = {
        method: 'DELETE',
        url: `http://ec2-54-160-45-255.compute-1.amazonaws.com:8080/v1/products/${id}`,
        headers: {
